...
let spec =
  Arg.align
    [
      ( "-I",
        Arg.String (fun i -> Clflags.include_dirs := i :: !Clflags.include_dirs),
        " includes a directory where to look for interface files" );
      ( "-rectypes",
        Arg.Set Clflags.recursive_types,
        " activates recursive types" );
      ( "-left2right",
        Arg.Set Mytools.use_left_to_right_order,
        " use the left-to-right evaluation order for sub-expressions, instead \
         of OCaml order" );
      ...
      ( "-only_normalize",
        Arg.Set only_normalize,
        " only generate the .cmj file, and attempt normalization, not the .v \
         file" );
      ( "-deep_embedding",
        Arg.Set Mytools.generate_deep_embedding,
        " generate the deep embedding of the source programs" );
      ( "-def_encoders",
        Arg.Set Mytools.generate_encoders,
        " generate the definition of the encoders for each type" );
      ("-debug", Arg.Set is_tracing, " trace the various steps");
      ( "-width",
        Arg.Set_int Print_coq.width,
        " set pretty-printing width for the .v file" );
      ( "-where",
        Arg.Unit Cfml_config.print_libdir,
...
let _ =
  ...
  if !Clflags.nopervasives && Filename.basename sourcefile <> "Pervasives.ml"
  then
  ...
  let debugdirBase =
    Filename.concat debugdir (String.capitalize_ascii basename)
  in
  (*  FAILURE ON WINDOWS   *)
  ...
  let ((typedtree2, _) : Typedtree.structure * Typedtree.module_coercion) =
    let fail () =
      failwith
        (Printf.sprintf
           "Could not typecheck the normalized source code\n\
            Check out the file %s_normalized.ml."
           debugdirBase)
    in
    try
  ...
  file_put_contents
    (debugdirBase ^ "_normalized_typed.ml")
    (Print_tast.string_of_structure typedtree2);
  ...
  let cftops =
    try Characteristic.cfg_file !no_mystd_include typedtree2 with
