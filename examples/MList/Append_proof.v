...
Lemma MList_contents_cons : forall A (EA :Enc A) x t (L: list A),
...
Lemma MList_cons : forall A (p : loc) p' (EA : Enc A) (L : list A) x,
...
Proof using.
  ...
  + xchange MList_eq ;=> v2. xapp. xapp.
    xchange (MList_contents_iff E Nil) ;=> [H1 H2]. intros.
    rewrite H1. 2: auto.
    rew_list. xchanges <- MList_eq p1. simpl.
    apply haffine_hpure.
